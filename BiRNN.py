...
#     test_data.write_ids_pickle()
    ...
    # 初始化模型并移动到指定设备
    model = BiRNN(train_max_ids + 1, embed_size, num_hiddens, len(train_data.class_indices)).to(device)
    ...
    for epoch in range(epochs):
        ...
        if val_accuracy > best_val_accuracy:
            ...
            torch.save(best_model_weights, "./weights/birnn_best_model.pth")
            print(f"New best validation accuracy: {best_val_accuracy:.4f}. Model saved.")
        
        torch.save(model.state_dict(), "./weights/birnn_latest_model.pth")

    # 加载最佳模型参数
    model.load_state_dict(torch.load("./weights/birnn_best_model.pth"))
